...
# ── Derive state FIPS from tract FIPS ────────────────────────────────────────
# Tract GEOID is an 11-digit code: SSCCCTTTTTT (2-digit state + 3-digit county + 6-digit tract)
df[:, :STATE] = div.(df[:, :TRACT], 1000000000);
...
for st in states
    ...
    state_results[:, :N_TRACTS] .= nrow(df_st)
...
println("=" ^ 80)
println("TRACT-LEVEL CORRELATIONS BY STATE")
println("=" ^ 80)
...
for st in states
    ...
    n = first(state_df[:, :N_TRACTS])

    println("\n── $state_name (FIPS: $st, N=$n tracts) ──")
...
for index in indices
    ...
    for st in states
        ...
        if length(valid) >= 5
            push!(rank_df, (
                st,
                state_name,
                first(all_state_results[mask, :N_TRACTS]),
                mean(valid),
                mean(abs.(valid)),
                median(valid)
            ))
    ...
    println("\n── Rankings for $index ──")
    println(rank_df[:, [:RANK, :STATE_NAME, :N_TRACTS, :MEAN_COR, :MEAN_ABS_COR, :MEDIAN_COR]])
end
...
# ── Export results ───────────────────────────────────────────────────────────
CSV.write("derived_data/07c_tract_by_state_correlations.csv", all_state_results)
println("\nResults saved to derived_data/07c_tract_by_state_correlations.csv")
