...
transform!(nri_data_zip, :POPULATION => ByRow(x -> Float64(x)) => :POPULATION)
transform!(nri_data_zip, :BUILDVALUE => ByRow(x -> Float64(x)) => :BUILDVALUE)
transform!(nri_data_zip, :AGRIVALUE => ByRow(x -> Float64(x)) => :AGRIVALUE)
...
mkpath("derived_data/county")
mkpath("derived_data/zcta")
mkpath("derived_data/tract")

CSV.write("derived_data/county/fema_eal.csv", nri_data_county)
CSV.write("derived_data/zcta/fema_eal.csv", nri_data_zip)
CSV.write("derived_data/tract/fema_eal.csv", nri_data_tract)
